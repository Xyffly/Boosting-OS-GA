...
def plot_confusion_matrix(cm,lables_name,title):
    ...
    plt.savefig(title + 'Catboost.tiff')
...
for i in range(20,200,10):
    ...
    CatBoostClassifier_model = GridSearchCV(estimator, param_grid, scoring='accuracy', cv=10)
    start = time.time()
    CatBoostClassifier_model.fit(X_train, Y_train)
    end = time.time()
    # 查看最佳分数
    print(CatBoostClassifier_model.best_score_)  # 0.7088001891107445
    # 查看最佳参数
    print(CatBoostClassifier_model.best_params_) #{'depth': 4, 'iterations': 300, 'l2_leaf_reg': 1, 'learning_rate': 0.15}

    model = CatBoostClassifier_model.best_estimator_
    ...
    dataex = openpyxl.load_workbook('Catboost输出结果.xlsx')
    table = dataex.active
    nrows = table.max_row # 获得行数
    for i in range(len(dataresult)):
        table.cell(nrows + 1, i + 1).value = dataresult[i]
    dataex.save('Catboost输出结果.xlsx')
