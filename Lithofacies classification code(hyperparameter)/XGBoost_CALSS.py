...
def plot_confusion_matrix(cm,lables_name,title):
    ...
    plt.savefig(title + 'XGBoost.tiff')
...
for i in range(20,150,10):
    ...
    XGBClassifier_model = GridSearchCV(estimator, param_grid, scoring='accuracy', cv=10 ,verbose=1 ,n_jobs=-1)
    start = time.time()
    XGBClassifier_model.fit(X_train,Y_train)
    end = time.time()
    print('最优参数：',XGBClassifier_model.best_params_)
    #最优参数： {'eval_metric': 'auc', 'learning_rate': 0.2, 'max_depth': 6, 'n_estimators': 100}
    #最优参数： {'eval_metric': 'auc', 'learning_rate': 0.2, 'max_depth': 9, 'n_estimators': 50}
    model = XGBClassifier_model.best_estimator_
    ...
    dataex = openpyxl.load_workbook('XGBoost输出结果.xlsx')
    table = dataex.active
    nrows = table.max_row # 获得行数
    for i in range(len(dataresult)):
        table.cell(nrows + 1, i + 1).value = dataresult[i]
    dataex.save('XGBoost输出结果.xlsx')