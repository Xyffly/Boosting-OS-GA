...
for i in range(20,150,10):
    ...
    #cb_model = GridSearchCV(model, params, scoring="accuracy", cv=3, n_jobs=-1)
    LGBMClassifier_model = GridSearchCV(estimator, param_grid, scoring='accuracy', cv=10, verbose=1, n_jobs=-1)
    start = time.time()
    LGBMClassifier_model.fit(X_train, Y_train)
    end = time.time()
    # 查看最佳分数
    print(LGBMClassifier_model.best_score_)  # 0.7088001891107445
    # 查看最佳参数
    print(LGBMClassifier_model.best_params_)  # {'depth': 4, 'iterations': 500, 'l2_leaf_reg': 9, 'learning_rate': 0.15}

    model = LGBMClassifier_model.best_estimator_
    ...
    dataex = openpyxl.load_workbook('LightGBM输出结果.xlsx')
    table = dataex.active
    nrows = table.max_row  # 获得行数
    for i in range(len(dataresult)):
        table.cell(nrows + 1, i + 1).value = dataresult[i]
    dataex.save('LightGBM输出结果.xlsx')
