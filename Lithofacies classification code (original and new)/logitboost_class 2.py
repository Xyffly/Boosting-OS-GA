...
#读取数据并将数据转换为array格式
data_files = []
data_files = pd.read_csv("C:/Users/Administrator.DESKTOP-G2KEBOV/Desktop/SMOTE数据/4features949original data.csv")
data_files2 = pd.read_csv("C:/Users/Administrator.DESKTOP-G2KEBOV/Desktop/SMOTE数据/596new data.csv")
#设定为函数
...
estimator = LogitBoost()
param_grid = {"n_estimators": [i for i in range(20,101,20)],
              "learning_rate": [0.2, 0.4, 0.6, 0.8],
              }

AdaBClassifier_model = GridSearchCV(estimator, param_grid, scoring='accuracy', cv=10 ,verbose=1 ,n_jobs=-1)


AdaBClassifier_model.fit(X_train,Y_train)
print('最优参数：',AdaBClassifier_model.best_params_)
#最优参数：
model = AdaBClassifier_model.best_estimator_
