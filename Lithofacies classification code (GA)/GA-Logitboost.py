...
def train_evaluate(ga_individual_solution):
    ...
    estimator = LogitBoost(n_estimators = n_estimators, learning_rate=learning_rate, random_state=42)
    ...
    print("F1分数:",f1,"\n")
    #to excel
    dataresult = [n_estimators, learning_rate, precision, recall, f1]
    dataex = openpyxl.load_workbook('GA-Logitboost输出结果.xlsx')
    table = dataex.active
    nrows = table.max_row # 获得行数
    for i in range(len(dataresult)):
        table.cell(nrows + 1, i + 1).value = dataresult[i]
    dataex.save('GA-Logitboost输出结果.xlsx')
    dataex.close()
    return precision,
