...
def train_evaluate(ga_individual_solution):
    ...
    estimator = GradientBoostingClassifier(n_estimators = n_estimators, learning_rate=learning_rate,max_depth=max_depth, random_state=42)
    ...
    dataex = openpyxl.load_workbook('GA-GBDT输出结果.xlsx')
    table = dataex.active
    nrows = table.max_row # 获得行数
    for i in range(len(dataresult)):
        table.cell(nrows + 1, i + 1).value = dataresult[i]
    dataex.save('GA-GBDT输出结果.xlsx')
    dataex.close()
    return precision,
